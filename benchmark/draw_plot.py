...
def read_result_from_file(filename):
    ...
    with open(filename, 'r') as file:
        ...
        for i in range(nq):
            ...
            topk = int(line[0])
            time = float(line[1])
            topk_result.append((time,[]))
            for j in range(topk):
               line = file.readline().split(' ')
               id = int(line[0])
               topk_result[i][1].append(id)
    return topk_result


def calc_recall(filename, baseline):
    result_metric = []
    topk_results = read_result_from_file(filename)
    baseline_results = read_result_from_file(baseline)

    for top_result, baseline_result in zip(topk_results, baseline_results):
        time, topk = top_result
        _, baseline_topk = baseline_result
        qps = len(topk) / time
        recall_number = 0
        for id in topk:
...
if __name__ == "__main__":
    linestyle = create_linestyles(["ip", "l2"])
    metrics_datas = calc_recall('result.txt', 'baseline.txt')
    data = {"ip": metrics_datas}
