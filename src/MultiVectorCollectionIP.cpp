...
Status
MultiVectorCollectionIP::CreateCollection(std::vector<int64_t> dimensions,
...
MultiVectorCollectionIP::Insert(const std::vector<milvus::multivector::RowEntity> &entity_arrays,
    ...
    for (auto i = 0; i < nq; ++i) {
        ...
        auto &target_row_entity = new_arrays[i];
...
Status
MultiVectorCollectionIP::Delete(std::vector<int64_t> &id_arrays) {
    return this->conn_ptr_->DeleteEntityByID(this->collection_name_, id_arrays);
}

Status
MultiVectorCollectionIP::CreateIndex(milvus::IndexType index_type, std::string extra_params) {
...
Status
MultiVectorCollectionIP::Search(std::vector<float> weight,
