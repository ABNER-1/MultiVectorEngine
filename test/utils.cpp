...
writeBenchmarkResult(const milvus::TopKQueryResult& topk_query_result,
    ...
    for (auto& result : topk_query_result) {
        for (int i = 0; i < topk; ++i) {
            if (i > result.ids.size())out << -1 << std::endl;
	    else out << result.ids[i] << " " << result.distances[i] << std::endl;
        }
    }
    out.close();
}
...
testIndexType(std::shared_ptr<milvus::multivector::MultiVectorEngine> engine,
    ...
    */
    ...
    assert_status(engine->SearchBase(collection_name, weights, query_entities, topk, query_json, topk_result, row_data));
